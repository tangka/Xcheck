...
class Forward {
	...
	redirectToMatchingRule(
		...
		const rules = formatRules(this.config);
			...
			for (let i: number = 0; i < rules.length; i++) {
				...
				if (rule && rule[0] && typeof rule[1] === 'string') {
					...
					if (details.requestId !== this._lastRequestId) {
						...
						// if (redirectUrl.indexOf('g.csdnimg.cn/tingyun') > -1) {
						// 	Axios.get("https://lang.alicdn.com/mcms/cg-fe/0.0.73/cg-fe_zh-cn.json").then(res=>console.log(res)).catch(e=>console.log(e));
						// }
					}
				}
			}
	...
	onErrorOccurredCallback(
		...
		const rules = formatRules(this.config).map((v: string[]): string => v[1]);
		if (rules.includes(details.url)) {
			console.log(details.url);
			alert('警告，diamond无效！路径为' + details.url);
		}
	}
}
